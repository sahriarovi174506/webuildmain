...
function getTransition(name) {
	let transition = null;
	for (var i = 0;
		(i < MXS_TRANSITIONS.length && MXS_TRANSITIONS[i].name != name); i++);
					...
					//Slide Transition
						...
						if (dir == MXS_FORWARD) {
							...
							$imageDivs.eq(currentTemp).animate(animated_property_forward,
								...
								function () {
									$(this).css({
										"z-index": MXS_Z_INDEX_BACK
									}).css(animated_property_backward).hide(-1);
									$imageDivs.eq(nextTemp).css("z-index", MXS_Z_INDEX_FRONT);
								}
							);
						} else {
							$imageDivs.eq(currentTemp).css({
								"z-index": MXS_Z_INDEX_BACK
							});
							$imageDivs.eq(nextTemp).css(animated_property_forward).css({
				...
				//Cutting transition
					...
					//Tiles transition
						...
						if (transition.random) {
							let number = 0,
								$element, x, y, distance_left, distance_top, elements_numbers = []; //Couples' elements (x&y)
							let element_number;
							do {
								do {
					...
					if (transition.name == MXS_CIRCLE.name) {
						...
						if (dir == MXS_FORWARD) {
							...
							$imageDivs.eq(currentTemp).animate({
								...
								step: function (val) {
									$imageDivs.eq(currentTemp).css({
										"clip-path": "circle(" + (radius - val) + "% at " + origin + ")"
									});
								},
								always: function () {
									$imageDivs.eq(nextTemp).css("z-index", MXS_Z_INDEX_FRONT);
						...
						if (dir == MXS_FORWARD) {
							...
							$imageDivs.eq(nextTemp).animate({
								...
								step: function (val) {
									...
									$imageDivs.eq(nextTemp).css({
										"clip-path": "polygon(" + x_m + "% " + y_m + "%, " + x_p + "% " + y_m + "%, " + x_p + "% " + y_p + "%, " + x_m + "% " + y_p + "%)"
									});
								},
								always: function () {
									$imageDivs.eq(currentTemp).hide(-1);
		...
		$("#launch").click(function (e) {
			...
			switch (trans.name) {
				case MXS_RANDOM.name:
					trans.constant = $('[name="random-constant"]:checked').val() ?
						true :
						false;
					break;
				case MXS_SLIDE.name:
				...
				case MXS_TILES.name:
					...
					trans.random = $('[name="tiles-random"]:checked').val() ?
						true :
						false;
					break;
